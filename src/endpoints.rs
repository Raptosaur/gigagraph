        ...
        Lang::Rust => {
            ...
            if has(fid, &["actix"]) {
                ...
                for t in calls {
                    ...
                    let handler = handler_from_ident(t, func, file, functions, name_index)
            ...
            for call in calls {
                ...
                // handler resolution gets import-following via `file`.
                for v in verbs {
                    let method = rust_verb(&v.name).unwrap();
                    let handler = handler_from_ident(v, func, file, functions, name_index)
